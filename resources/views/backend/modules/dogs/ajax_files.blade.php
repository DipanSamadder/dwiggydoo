        ...
        @if(is_array($data) || count($data) > 0 )
            ...
            @php $key++; @endphp
                <tr>
                    ...
                    <td>
                        <a href="#" target="_blank">{{ $value->name }}</a><br>
                    ...
                    <td>
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="{{$value->slug }}" onchange="DSLDStatusUpdate('{{ $value->id }}','{{ $value->status == 1 ? 0 : 1  }}', '{{ route('dogs.status') }}','{{ csrf_token() }}')" @if($value->status == 1) checked @endif >
                    ...
                    <td>
                        <p class="text-center mb-0 action_items">
                            ...
                            @if(dsld_have_user_permission('posts_edit') == 1)
                            <a href="{{ route('dogs.edit', [$value->id]) }}"  class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            
                            @if(dsld_have_user_permission('posts_delete') == 1)
                            <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-danger" onclick="DSLDDeleteAlert('{{ $value->id }}','{{ route('dogs.destory') }}','{{ csrf_token() }}')">
