        ...
        @if(is_array($data) || count($data) > 0 )
            ...
            @php $key++; @endphp
                <tr>
                    ...
                    <td>
                        @if($value->banner > 0)
                            <img src="{{ dsld_uploaded_file_path($value->banner) }}" alt="{{ dsld_upload_file_title($value->banner) }}" class="page_banner_icon">
                        @else
                    ...
                    <td>
                        <p class="text-center mb-0 action_items">
                            ...
                            @if(dsld_have_user_permission('pages_edit') == 1)
                            <a href="{{ route('blogs.edit', ['id'=>$value->id, 'lang'=>env('DEFAULT_LANGUAGE')]) }}"  class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
