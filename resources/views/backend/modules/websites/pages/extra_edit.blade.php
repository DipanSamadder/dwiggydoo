    ...
    @csrf 
        ...
        @foreach (json_decode($sec->meta_fields) as $key2 => $element)
            ...
            @if ($element->type == 'text')
                ...
                @elseif ($element->type == 'mission')
                @php
                    $page_mission_bg = $page_meta_key."_bg";
                    $page_mission_vision = $page_meta_key."_vision";
                    $page_mission_mission = $page_meta_key."_mission";
                @endphp
                <div class="row clearfix">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-8">
                        <div class="form-group">
                            <input type="hidden" name="type[]" value="{{ $page_mission_bg }}">
                            <select class="form-control show-tick ms select2" name="{{ $page_mission_bg }}" onchange="is_edited()">
                                <option value="">-- Please select --</option>
                                
                                @foreach(App\Models\Upload::where('user_id', Auth::user()->id)->orderBy('id', 'desc')->get() as $key => $value)
                                    <option value="{{ $value->id }}" @if(dsld_page_meta_value_by_meta_key($page_mission_bg, $data->id) == $value->id) selected @endif>({{ $value->id }}) - {{ $value->file_title}} - {{ $value->extension}}</option>
                                @endforeach
                            </select>
                            @if(dsld_page_meta_value_by_meta_key($page_mission_bg, $data->id) != '')
                            <div class="image mt-2">
                                <img src="{{ dsld_uploaded_asset(dsld_page_meta_value_by_meta_key($page_mission_bg, $data->id)) }}"  alt="{{ dsld_upload_file_title(dsld_page_meta_value_by_meta_key($page_mission_bg, $data->id)) }}" class="img-fluid" width="100">
                            </div> 
                            @endif 
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-8">
                        <div class="form-group">
                            <input type="hidden" name="type[]" value="{{ $page_mission_vision }}">
                            <textarea  class="summernote" name="{{ $page_mission_vision }}"><?php $str = dsld_page_meta_value_by_meta_key($page_mission_vision, $data->id); echo htmlspecialchars_decode($str); ?></textarea >  
                        </div><small>Meta Key: {{ $page_mission_vision }}</small>
                    </div>
                    <div class="col-lg-2 col-md-2 col-sm-4 form-control-label">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-8">
                        <div class="text_repeter{{ $sec->id }}_{{ $key2 }}">
                            <input type="hidden" name="type[]" value="{{ $page_mission_mission }}">
                            
                            @if(dsld_page_meta_value_by_meta_key($page_mission_mission, $data->id) != '')
                                @foreach(json_decode(dsld_page_meta_value_by_meta_key($page_mission_mission, $data->id), true) as $key3 => $value) 
                                    
                                    <div class="row clearfix">
                                        <div class="col-sm-10">
                                            <div class="form-group">
                                                <input type="text" class="form-control"  name="{{ $page_mission_mission }}[]" placeholder="{{ ucfirst($element->label) }}" value="{{ $value }}">  
                                            </div>
                                        </div>
                                        <div class="col-auto">
                        ...
                        <small>Meta Key: {{ $page_mission_mission }}</small>
                    </div>
                </div>
                <div class="row clearfix">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-10">
                        <div class="input-group mb-4">
                            <button
                                ...
                                data-content='<div class="row clearfix">
                                    <div class="col-sm-10">
                                        <div class="form-group"><input type="text" class="form-control"  name="{{ $page_mission_mission }}[]" placeholder="{{ ucfirst($element->label) }}">  
                                        </div>
                                    </div>
                                    <div class="col-auto">
                                        <button type="button" class="btn btn-sm btn-danger" data-toggle="remove-parent" data-parent=".row">
                        ...
                        </div><small>Meta Key: {{ $page_mission_mission }}</small>
                    </div>
                </div>
