    ...
    <div class="row clearfix">
        ...
        <div class="col-lg-4">
                ...
                <div class="body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="template" id="template" onchange="is_edited()">
                            ...
                            <option value="school_details"  @if($data->template == 'school_details') selected @endif>School Details</option>
                        </select>                             
                    </div>
            ...
            <div class="card mb-0">
                ...
                <div class="body">
                    <div class="form-group">
                        <label class="form-label">Thumbnail </label><a class="btn btn-primary text-white" onclick="media_file_get('{{ @$data->thumbnail }}','put_image2', 0)"><i class="zmdi zmdi-collection-image"></i></a><div class="put_image2">@if(isset($data->thumbnail))<strong>Selected Image:</strong><i> {{ @$data->thumbnail }}</i>@endif</div>
                        
                        <input type="hidden" class="put_image2" name="thumbnail" id="thumbnail" value="{{ @$data->thumbnail }}" onchange="is_edited()">
                        @if($data->thumbnail > 0)
                        <div class="image mt-2">
                            <img src="{{ dsld_uploaded_asset($data->thumbnail) }}"  alt="{{ dsld_upload_file_title($data->thumbnail) }}" class="img-fluid">
                        </div> 
                        @endif  
            ...
            <div class="card mb-0">
                ...
                @endphp
                    ...
                    <ul class="nav nav-tabs p-0 mb-3 nav-tabs-success" role="tablist">
                        ...
                        @foreach($departments as $key => $sec)
                            <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#page_section-{{ $key }}-{{ $sec->id }}" onclick="edit_department_pages_section('{{ $sec->id }}', '{{ $data->id }}')"> {{ $sec->title }} </a></li>
                    ...
                    <div class="tab-content">
                        ...
                        <div role="tabpanel" class="tab-pane  @if($key == 0 ) in active @endif" id="page_section-{{ $key }}-{{ $sec->id }}">
                            <div id="edit_department_pages_section_body{{ $sec->id }}"></div>
                        </div>
...
<div class="school_fields" style="display:none">
    <hr>
                ...
                <div class="body">
                    ...
                    <ul class="nav nav-tabs p-0 mb-3 nav-tabs-success" role="tablist">
                        @foreach($school as $key => $sec)
                            <li class="nav-item"><a class="nav-link @if($key == 0 ) active @endif" data-toggle="tab" href="#page_section-{{ $key }}-{{ $sec->id }}" onclick="edit_school_pages_section_body('{{ $sec->id }}', '{{ $data->id }}')"> {{ $sec->title }} </a></li>
                        @endforeach
                    </ul>
                    ...
                    <div class="tab-content">
                        ...
                        <div role="tabpanel" class="tab-pane  @if($key == 0 ) in active @endif" id="page_section-{{ $key }}-{{ $sec->id }}">
                            <div id="edit_school_pages_section_body{{ $sec->id }}"></div>
                        </div>
                        @endforeach
...
<script>
     ...
     function edit_department_pages_section(section_id, page_id){
        // $('#edit_pages_section').modal('show');
        $('#edit_department_pages_section_body'+section_id).html('<center><img src="{{ dsld_static_asset('backend/assets/images/circle-loading.gif') }}" style="max-width:100px" ></center>');
        ...
        $.ajax({
            ...
            success: function(d) {
                $('#edit_department_pages_section_body'+section_id).html(d);
            }
        });
    }

    function edit_school_pages_section_body(section_id, page_id){
            // $('#edit_pages_section').modal('show');
            $('#edit_school_pages_section_body'+section_id).html('<center><img src="{{ dsld_static_asset('backend/assets/images/circle-loading.gif') }}" style="max-width:100px" ></center>');

            $.ajax({
                url: "{{ route('schools.edit_extra') }}",
                type: "post",
                cache : false,
                data: {'_token':'{{ csrf_token() }}', 'section_id':section_id, 'id':page_id},
                success: function(d) {
                    $('#edit_school_pages_section_body'+section_id).html(d);
                }
            });
        }
    ...
    $(document).ready(function(){
        ...
        if($('#template').val() == 'school_department') { 
            ...
            $('.school_fields').hide(); 
        }else if ($('#template').val() == 'school_details') { 
            $('.school_fields').show();
    ...
    $('#template').on('change', function(){
        if ($(this).val() == 'school_department') { 
            ...
            $('.department_fields').show();    
            $('.school_fields').hide(); 
        }else if ($(this).val() == 'school_details') { 
