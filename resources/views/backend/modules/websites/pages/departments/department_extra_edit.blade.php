        ...
        @foreach (json_decode($sec->meta_fields) as $key2 => $element)
            ...
            <div class="row clearfix">
                ...
                <div class="col-lg-10 col-md-10 col-sm-8">
                    <div class="form-group">
                        <input type="hidden" name="type[]" value="{{ $page_meta_key }}">
                        <select class="form-control w-100  ms select2 mr-2" name="{{ $page_meta_key }}[]" multiple>
                            <option value="">-- Please select --</option>
                            

                            @foreach(App\Models\Page::where('status', 1)->where('type', 'leadership')->where('cat_type', 'faculty_data')->orderBy('order', 'asc')->get() as $key => $value)
                                <option value="{{ $value->id }}" @if(is_array(json_decode(dsld_page_meta_value_by_meta_key($page_meta_key, $data->id), true)))  @if(in_array($value->id, json_decode(dsld_page_meta_value_by_meta_key($page_meta_key, $data->id), true))) selected @endif @endif> ({{ $value->id }}) {{ $value->title }}
                                </option>
                            @endforeach
                        </select> 
                        <small>Meta Key: {{ $page_meta_key }}</small>
                    ...
                    <div class="form-group">
                        <input type="hidden" name="type[]" value="{{ $page_meta_key }}">
                            <select class="form-control show-tick ms select2" name="{{ $page_meta_key }}[]"  multiple>
                                <option value="">-- Please select --</option>
                                @foreach(App\Models\page::where('status', 1)->where('type', 'program_page')->where('level', 3)->orderby('order', 'desc')->get() as $key => $value)
                                    ...
                                    <div class="row clearfix">
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <div class="image mt-2">
                                                    <img src="{{ dsld_uploaded_asset_not_default(json_decode(dsld_page_meta_value_by_meta_key($page_meta_key_repeter_img, $data->id), true)[$key3]) }}"  alt="{{ dsld_upload_file_title(json_decode(dsld_page_meta_value_by_meta_key($page_meta_key_repeter_img, $data->id), true)[$key3]) }}" class="img-fluid" width="100">
            ...
            @elseif ($element->type == 'file')
                ...
                <div class="row clearfix">
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-8">
                        <div class="form-group">
                            <input type="hidden" name="type[]" value="{{ $page_meta_key }}">
                            <label class="form-label">Banner </label><a class="btn btn-primary text-white" onclick="media_file_get('{{ dsld_page_meta_value_by_meta_key($page_meta_key, $data->id) }}','{{ $page_meta_key }}', 0)"><i class="zmdi zmdi-collection-image"></i></a><div class="{{ $page_meta_key }}">@if(isset($data->banner))<strong>Selected Image:</strong><i> {{ @$data->banner }}</i>@endif</div>
