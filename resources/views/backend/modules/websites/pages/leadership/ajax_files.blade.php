...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                @if(dsld_have_user_permission('leaderships_edit') == 1)
                <th>Status</th>
                @endif

                @if(dsld_have_user_permission('leaderships_delete') == 1 || dsld_have_user_permission('leaderships_edit') == 1)
                <th>Action</th>
                @endif
            </tr>
        </thead>
        <tfoot>
            <tr class="text-center">
                ...
                @if(dsld_have_user_permission('leaderships_delete') == 1 || dsld_have_user_permission('institute_type_edit') == 1)
                <th>Action</th>
                @endif
            </tr>
        </tfoot>
        ...
        @if(is_array($data) || count($data) > 0 )
            @foreach($data as $key => $value)
                ...
                <tr>
                    ...
                    <td>
                            @if(dsld_have_user_permission('leaderships_edit') == 1)
                            <a href="javascript:void(0)"  onclick="edit_lg_modal_form({{ $value->id }}, '{{ route('leaderships.edit') }}', 'Category');" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            @if(dsld_have_user_permission('leaderships_delete') == 1)
