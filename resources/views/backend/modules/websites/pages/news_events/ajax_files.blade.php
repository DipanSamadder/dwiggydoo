...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                @if(dsld_have_user_permission('newsevents_edit') == 1)
                <th width="10">Status</th>
                @endif

                @if(dsld_have_user_permission('newsevents_delete') == 1 || dsld_have_user_permission('newsevents_edit') == 1)
        ...
        @if(is_array($data) || count($data) > 0 )
            @foreach($data as $key => $value)
                ...
                <tr>
                    ...
                    <td width="15">
                            @if(dsld_have_user_permission('newsevents_edit') == 1)
                            <a href="javascript:void(0)"  onclick="edit_lg_modal_form({{ $value->id }}, '{{ route('news_event.edit') }}', 'Category');" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            @if(dsld_have_user_permission('newsevents_delete') == 1)
