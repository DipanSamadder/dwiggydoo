...
<table class="table table-bordered table-striped table-hover dataTable">
    <thead>
            <tr class="text-center">
                ...
                @if(dsld_have_user_permission('clubs_edit') == 1)
                <th>Status</th>
                @endif

                @if(dsld_have_user_permission('clubs_delete') == 1 || dsld_have_user_permission('clubs_edit') == 1)
                <th>Action</th>
                @endif
            </tr>
        </thead>
        <tfoot>
            <tr class="text-center">
                ...
                @if(dsld_have_user_permission('clubs_delete') == 1 || dsld_have_user_permission('institute_type_edit') == 1)
                <th>Action</th>
                @endif
            </tr>
        </tfoot>
        ...
        @if(is_array($data) || count($data) > 0 )
            @foreach($data as $key => $value)
                ...
                <tr>
                    ...
                    <td>
                       Name: ({{ $value->id }}) <a href="{{ route('custom-pages.show_custom_page', [$value->slug]) }}" target="_blank"> <strong>{{ $value->title }}</strong></a><br>
                       <small><strong>Created_at: </strong>{{ $value->created_at }}</small><br>
                    ...
                    <td>
                            @if(dsld_have_user_permission('clubs_edit') == 1)
                            <a href="javascript:void(0)"  onclick="edit_lg_modal_form({{ $value->id }}, '{{ route('clubs.edit') }}', 'Category');" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-primary">
                                <i class="zmdi zmdi-edit"></i>
                            </a>
                            @endif
                            @if(dsld_have_user_permission('clubs_delete') == 1)
